  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        ...
        const spanishRecipes = response.data.filter(recipe => recipe.category === "Spanish");
        setRecipes(spanishRecipes);
        setLoading(false);
      } catch (error) {
    ...
    <section className="page-recipes pb-4 pt-4">
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item active" aria-current="page">Spanish Recipes</li>
        </ol>
      </nav>

      <div className='mb-5'>
        <h2 className="search-results-heading page-recipes-heading animated">
          <FaFish className="search-icon" /> {/* Replace with your chosen icon */}
          Spanish Recipes
...
export default SpanishRecipesPage;