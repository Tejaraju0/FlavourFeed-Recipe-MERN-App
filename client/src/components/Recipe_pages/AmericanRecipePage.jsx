  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        ...
        const americanRecipes = response.data.filter(recipe => recipe.category === "American");
        setRecipes(americanRecipes);
        setLoading(false);
      } catch (error) {
    ...
    <section className="page-recipes pb-4 pt-4">
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item active" aria-current="page">American Recipes</li>
        </ol>
      </nav>

      <div className='mb-5'>
        <h2 className="search-results-heading page-recipes-heading animated">
          <FaFlagUsa className="search-icon" /> {/* Replace with your chosen icon */}
          American Recipes
      ...
      {loading ? (
        ...
        <p>No American recipes found.</p>
      )}
    </section>
  );
}

export default AmericanRecipesPage;
